...
def init_background(occ_list):
    ...
    # make QD
    attractors.append(Attractor(pygame.Vector2(display_width / 2, display_height / 4)))

    # make bath sites
    NBATH = (len(occ_list) - 2) // 2
    for i in range(NBATH):
        attractors.append(Attractor(pygame.Vector2((i+1)*display_width/(NBATH+1), 3*display_height/4)))
...
def init_boids(occ_list):
    ...
    for i, n in enumerate(fUp):
        for _ in range(n):
            boids.append(Boid(attractorid=i, color=spinUpColor))
    
    for i, n in enumerate(fDown):
        for _ in range(n):
            boids.append(Boid(attractorid=i, color=spinDownColor))
...
def update_boids(dt):
    for boid in boids:
        boid.edges()
        boid.update(boids, attractors, dt)
        boid.draw()
...
def game_loop(occ_list) -> None:
    ...
    Vg = Slider(sx=100, sy=100, width=200, height=30, initialValue=0, valueMin=0, valueMax=10)
    X = Slider(sx=100, sy=200, width=200, height=30, initialValue=0, valueMin=0, valueMax=1 )
    ...
    while True:
        ...
        # make the ff. only run when slider is updated (or maybe mouse is clicked)
